...
<style>body{{ margin: 0; padding: 16px; background: white; font-family: sans-serif; }}</style>
    ...
    with sync_playwright() as p:
        ...
        for idx, (section, code) in enumerate(blocks, 1):
            ...
            try:
                ...
                try:
                    page.wait_for_function("window.rendered === true", timeout=10000)
                except Exception:
                    # Fallback: ждём появления SVG
...
def md_to_docx(md_path, out_path, diagrams_dir):
    ...
    doc.core_properties.title = "Screen Flow HTML_MeetingProtokol"

    # Заголовок документа (18 пт, жирный, по центру)
    add_document_title(doc, "Screen Flow HTML_MeetingProtokol")
...
def main():
    ...
    md_path = project_root / "artifacts" / "06-screens" / "SCREEN_FLOW.md"
    diagrams_dir = project_root / "artifacts" / "06-screens" / "diagrams"
    diagrams_dir.mkdir(parents=True, exist_ok=True)
    out_path = md_path.parent / "SCREEN_FLOW.docx"
